...
function getNodeSize(d) {
  if (d.type === 'type4') {
    return { width: 150, height: 60 }; // Adjust as needed
  } else {
    return { width: 120, height: 45 }; // Default size
  }
}
...
function treeBoxes(urlService, jsonData)
{
	...
	//jsonData.children[2].rectNode = { width: 150, height: 60, textMargin: 5 };
	...
	function drawTree(jsonData)
	{
		...
		var maxTreeWidth = breadthFirstTraversal(tree.nodes(root), function(currentLevel) {
			...
			currentLevel.forEach(function(node) {
				...
				if (node.type == 'type4') {
                node.color = purple;
              //  node.rectNode = { width: 150, height: 80, textMargin: 5 }; // Update rectNode dimensions for type 4
            } else {
               // node.rectNode = { width: 120, height: 45, textMargin: 5 }; // Default dimensions for other types
            }
				});
			});
			...
			height = maxTreeWidth * (rectNode.height + 30) +  - margin.right - margin.left;
	...
	// 1) ******************* Update the nodes *******************
		...
		/*var nodeEnterTooltip = nodesTooltip.enter().append('g')
	        ...
	        // rectNode = { width : 120, height :  function (d) { return d.type =='type4'?(80):60; }, textMargin : 5 }
	        //rectNode.height = function (d) { return d.type =='type4'?(rectNode.height):rectNode.height; }
		...
		.append('xhtml').html(function(d) {
					return '<div style="width: '
							...
							+ (rectNode.height - rectNode.textMargin * 2) + 'px; overflow: auto;" class="node-text wordwrap">'
		...
		.style('fill', 'white')
		.append("tspan")
    .text(function(d) { return 'Slot ID: ' + d.slotID; })
    .append("tspan")
    .attr('x', rectNode.width / 2 + tooltip.textMargin)
    .attr('dy', '1.5em')
    ...
    .attr('x', rectNode.width / 2 + tooltip.textMargin)
    .attr('dy', '1.5em')
    .text(function(d) { return 'Subsys_ID: ' + d.subsys_id; })
    .append("tspan")
    .attr('x', rectNode.width / 2 + tooltip.textMargin)
    .attr('dy', '1.5em')
    .text(function(d) { return 'Dev_path: ' + d.dev_path; })
    .append("tspan")
    .attr('x', rectNode.width / 2 + tooltip.textMargin)
    .attr('dy', '1.5em')
    .text(function(d) { return 'Init_time: ' + d.init_time; })
    .append("tspan")
    .attr('x', rectNode.width / 2 + tooltip.textMargin)
    .attr('dy', '1.5em')
    ...
    */
	...
	function openLogsPopup(logsContent) {
  //alert(logsContent);
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header bg-primary text-white">
          <h5 class="modal-title">Logs Content</h5>
